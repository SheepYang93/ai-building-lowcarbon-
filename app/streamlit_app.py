...
with tabs[3]:
    ...
    if f.exists():
        ...
        cols = [c for c in ["building_id", "building_type", "energy_archetype", "cause_candidate", "best_measure_family", "action_confidence", "annual_meter_reading", "energy_saving_20%", "action_priority"] if c in d.columns]
        if cols:
...
with tabs[4]:
    annual = st.number_input("Annual energy reading", min_value=0.0, max_value=1e9, value=1000000.0, step=10000.0)
    reduction = st.slider("Assumed reduction (%)", 0, 50, 20) / 100
    ef = st.number_input("Emission factor (kgCO₂e/kWh)", min_value=0.0, max_value=2.0, value=.5777, step=.01)
    st.metric("Scenario CO₂e", f"{annual * reduction * ef / 1000:,.1f} t")
    st.warning("Conditional scenario only: verify the meter unit and emission factor first.")
